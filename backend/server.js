...
// ── Debug endpoints (safe - no sensitive data exposed) ────────────────────────
...
// Test SSE streaming pipeline without AI
app.get('/api/debug/test-stream', (req, res) => {
  ...
  const words = ['Hello', ' from', ' Nova', ' AI!', ' Streaming', ' works', ' correctly.'];
  let i = 0;
  const iv = setInterval(() => {
    if (i < words.length) {
      ...
      res.write(`data: ${JSON.stringify({ type: 'done', messageId: 'test-123' })}\n\n`);
      clearInterval(iv);
      res.end();
    }
  }, 100);
