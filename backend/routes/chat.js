...
// Helper: get Anthropic client (lazy, so API key is read at request time not module load)
function getClient() {
...
router.post('/conversations/:id/messages', authMiddleware, async (req, res) => {
  ...
  const apiMessages = history.map(m => {
    // Attach images to the current user message
  ...
  try {
    ...
    // Use stream() which returns a MessageStream with async iteration support
    ...
    console.error('[Nova] Streaming error:', err.message);
