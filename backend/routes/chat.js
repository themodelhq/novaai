...
const PROVIDERS = {
  ...
  ollama_cloud: {
    tier: 'free',
    label: 'Ollama Cloud',
    // Ollama Cloud: OpenAI-compatible, no key needed, just use a placeholder
    url: () => 'https://ollama.com/api/chat',
    model: (userModel) => userModel || 'llama3.3',
    ...
    // Ollama uses {"message":{"content":"..."}} format per chunk
    parseChunk: (p) => {
      if (p.message?.content) return p.message.content;
      // also handle OpenAI-compat format
...
function buildProviderChain(userKeys) {
  ...
  const keyMap = {
    ...
    ollama_cloud:    'free',                              // no key required
