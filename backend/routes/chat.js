...
// ── Raw fetch streaming to Anthropic API ──────────────────────────────────────
// Uses Authorization: Bearer (gateway compatible) + x-api-key (direct API compatible)

async function streamAnthropicMessage({ apiKey, messages, onText }) {
  const apiUrl = process.env.ANTHROPIC_API_URL || 'https://api.anthropic.com/v1/messages';
  const model = process.env.ANTHROPIC_MODEL || 'claude-3-5-sonnet-20241022';

  console.log(`[Nova] Calling ${apiUrl} with model ${model}`);
  ...
  console.log(`[Nova] Anthropic response status: ${response.status}`);

  if (!response.ok) {
    const errText = await response.text();
    console.error(`[Nova] Anthropic API error body: ${errText}`);
    let errMsg = `API error ${response.status}`;
    try { errMsg = JSON.parse(errText).error?.message || errMsg; } catch (_) {}
    throw new Error(errMsg);
  }

  // Parse SSE stream — split on double newline (proper SSE boundary)
  ...
  console.log(`[Nova] Stream complete. Response length: ${fullText.length} chars`);
  return fullText;
}
...
router.post('/conversations', authMiddleware, (req, res) => {
  const id = uuidv4();
  const { title = 'New Conversation', model = 'claude-3-5-sonnet-20241022' } = req.body;
...
router.post('/conversations/:id/messages', authMiddleware, async (req, res) => {
  ...
  // Keep-alive ping every 15s so Render doesn't kill idle connection
  const keepAlive = setInterval(() => {
    try { res.write(': ping\n\n'); } catch (_) {}
  }, 15000);

  try {
    const fullText = await streamAnthropicMessage({
      apiKey,
    ...
    if (!fullText) {
      throw new Error('Empty response from AI — check your ANTHROPIC_API_KEY is valid');
    }
    ...
    sendEvent({ type: 'done', messageId: asstMsgId });
    res.end();
