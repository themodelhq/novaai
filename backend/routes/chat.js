...
router.post('/conversations', authMiddleware, (req, res) => {
  const id = uuidv4();
  const { title = 'New Conversation', model = 'claude-sonnet-4-5' } = req.body;
...
router.post('/conversations/:id/messages', authMiddleware, async (req, res) => {
  ...
  // SSE headers — flush immediately so client knows connection is open
  ...
  try {
    ...
    // messages.stream() returns a MessageStream synchronously — register
    // .on('text') BEFORE awaiting anything so no events are missed
    ...
    // Register text handler synchronously (before any await)
    stream.on('text', (text) => {
      fullText += text;
      sendEvent({ type: 'text', content: text });
    });

    // Now wait for the stream to finish
    await stream.done();
