...
router.post('/', authMiddleware, (req, res) => {
  const { openrouter, anthropic, openai, groq, mistral, google, active_provider, active_model } = req.body;
  const existing = db.prepare('SELECT * FROM api_keys WHERE user_id = ?').get(req.user.id);

  if (!existing) {
    db.prepare(`
      INSERT INTO api_keys (user_id, openrouter_key, anthropic_key, openai_key, groq_key, mistral_key, google_key, active_provider, active_model)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      req.user.id,
      openrouter || null, anthropic || null, openai || null,
...
router.post('/test', authMiddleware, async (req, res) => {
  ...
  if (!apiKey && provider !== 'ollama_cloud') {
    return res.json({ ok: false, error: `No API key saved for ${provider}.` });
  }
  ...
  try {
    ...
    if (provider === 'anthropic') {
      ...
      const d = await r.json();
      if (!r.ok) return res.json({ ok: false, error: d.error || `Error ${r.status}` });
      result = { ok: true, response: d.message?.content || '', tokens: { prompt: 0, completion: 0, total: 0 }, rawJson: d };
